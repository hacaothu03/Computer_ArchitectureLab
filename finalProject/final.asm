.eqv IN_ADRESS_HEXA_KEYBOARD 0xFFFF0012 #d/chi ban phim khi doc du lieu
.eqv OUT_ADRESS_HEXA_KEYBOARD 0xFFFF0014#ghi
.eqv KEY_CODE 0xFFFF0004 	# ASCII code from keyboard, 1 byte
.eqv KEY_READY 0xFFFF0000 	# =1 if has a new keycode ?
 				# Auto clear after lw
#-------------------------------------------------------------------------------
# Key value: gan gia tri hexa cho so tren bban phim
	.eqv KEY_0 0x11
	.eqv KEY_1 0x21
	.eqv KEY_2 0x41
	.eqv KEY_3 0x81
...
#		lengthPath variable
#-----------------------------------------------------------	
storePath:#Ý nghĩa: Lưu lại thông tin về đường đi của Marsbot vào mảng path.
#Đầu vào: biến nowHeading, lengthPath.
#Mảng path lưu thông tin về đường đi hay đúng hơn là thông tin về các cạnh của
#đường đi của Marsbot. Mỗi một cạnh gồm 3 thông tin: tọa độ x và y của điểm đầu
#tiên, hướng đi của cạnh đó.
	#backup
	addi $sp,$sp,4
	sw $t1, 0($sp)
	addi $sp,$sp,4
...
#-----------------------------------------------------------		
goBack:
#Ý nghĩa: điều khiển Marsbot đi ngược lại theo lộ trình nó đã đi và về điểm xuấtphát
#Đầu vào: mảng path lưu thông tin đường đi, biến lengthPath lưu kích cỡ của mảng path theo byte.
#Mảng path lưu thông tin đường đi. Mỗi thông tin về 1 cạnh gồm tọa độ x và y và
#hướng đi - 3 số nguyên. Do đó mỗi thông tin đường đi sẽ chiếm 12 byte. Do đó
#lengthPath sẽ có giá trị là bội của 12.
#Mỗi khi muốn quay ngược lại và đi về điểm đầu tiên của 1 cạnh trên đường đi, ta
#sẽ lấy hướng đi của cạnh đó và đi ngược lại, đến khi nào gặp điểm có tọa độ như
#đã lưu thì kết thúc việc đi ngược trên cạnh đó, tiếp tục trên cạnh khác.
	#backup
	addi $sp,$sp,4
	sw $s5, 0($sp)
	addi $sp,$sp,4
...
goCheo:
	...
	la $s5, nowHeading
	lw $s6, 0($s5)	#$s6 is heading at now
	addi $s6, $s6, 45 #increase heading by 90*
	sw $s6, 0($s5) # update nowHeading
	#restore
	lw $s6, 0($sp)
	addi $sp,$sp,-4
...
# param[in] none
#-----------------------------------------------------------				
removeControlCode:#xóa xâu inputControlCode (nơi lưu trữ code điều khiển nhập vào).Lần lượt gán các kí tự trong xâu bằng ‘\0’.
